...
const SOURCES = {
    ...
    vegetable: "https://ourworldindata.org/grapher/vegetable-consumption-per-capita.csv?csvType=full&useColumnShortNames=false",
    calories: "https://ourworldindata.org/grapher/daily-per-capita-caloric-supply.csv?csvType=full&useColumnShortNames=false",
    obesity: "https://ourworldindata.org/grapher/share-of-adults-defined-as-obese.csv?csvType=full&useColumnShortNames=false",
    lifeExpectancy: "https://ourworldindata.org/grapher/life-expectancy.csv?csvType=full&useColumnShortNames=false",
    cvdDeaths: "https://ourworldindata.org/grapher/deaths-from-cardiovascular-disease-ghe.csv?csvType=full&useColumnShortNames=false",
    population: "https://ourworldindata.org/grapher/population.csv?csvType=full&useColumnShortNames=false"
};
const OUTPUT_PATH = node_path_1.default.resolve(process.cwd(), "public", "data", "nutrition-health-merged.json");
const MIN_YEAR = 2000;
const MAX_YEAR = 2022;
...
const isAggregateEntity = (entity) => {
...
const pickLatestByCountry = (series) => {
    const best = new Map();
...
const pickLatestSharedYearRate = (numerator, denominator) => {
    const numeratorMap = new Map();
    const denominatorMap = new Map();
    for (const row of numerator) {
        if (!numeratorMap.has(row.code))
            numeratorMap.set(row.code, new Map());
        numeratorMap.get(row.code).set(row.year, row);
    }
    for (const row of denominator) {
        if (!denominatorMap.has(row.code))
            denominatorMap.set(row.code, new Map());
...
const main = async () => {
    const [fruitSeries, vegetableSeries, caloriesSeries, obesitySeries, lifeSeries, cvdSeries, populationSeries] = await Promise.all([
    ...
    await promises_1.default.mkdir(node_path_1.default.dirname(OUTPUT_PATH), { recursive: true });
    await promises_1.default.writeFile(OUTPUT_PATH, JSON.stringify(payload, null, 2), "utf8");
